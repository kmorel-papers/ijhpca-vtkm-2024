...
Figure~\ref{fig:paraview-crusher} shows an example of ParaView running of Crusher, which is an early access test bed for the Frontier system, with VTK-m accelerated filters. As described in section \nameref{sec:adopting-kokkos}, VTK-m is using the Kokkos device adapter backend on this hardware. The bottom of the image shows the output of the \texttt(rocm-smi) command, which used to verify and monitor the GPU usage by the filters.
