...
\begin{abstract}
  %\assign{Hank, make an editing pass}
  \vtkm is a software library that enables scientific visualization on Exascale machines.
  This is challenging since 
  Exascale machines use GPUs as accelerator processors to provide the highest computational throughput available today, necessitating an algorithmic design that often deviates from previous generations of high-performance computers. 
  %\ollie{"classical computer" v.s. "quantum computer" ???}
  %\ken{Good point. I've rephrased.}
  In addition to providing scientific visualization algorithms for GPUs and other accelerator processors, \vtkm provides a framework that simplifies the implementation of new algorithms and provides a porting layer to work across multiple processor types.
  This paper captures the main challenges encountered to make scientific visualization available at the Exascale.
  ...
  Finally, we show how \vtkm helps scientific discovery for applications like fusion and particle acceleration that leverage an Exascale supercomputer.
...
\keywords{visualization, exascale, VTK-m, parallel, ECP}
